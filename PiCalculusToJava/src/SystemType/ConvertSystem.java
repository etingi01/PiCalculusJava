...
public class ConvertSystem {
	...
	public ConvertSystem(String file) throws ParserConfigurationException, SAXException, IOException {
	    ...
	    createAtomicGroupClass();
	    createChannelValueClass();
	}
	public static void createAtomicGroupClass() throws IOException{
		String fileName = "atomicGroup.java";
		@SuppressWarnings("resource")
		PrintWriter writer = new PrintWriter(fileName, "UTF-8");
		new File(fileName).createNewFile();	
		writer.println("import org.jcsp.lang.CSProcess;");
		writer.println("import org.jcsp.lang.Parallel;");
		writer.println("public class atomicGroup implements CSProcess  {");
		writer.println("\t public CSProcess[] parArray;");
		writer.println("\t public String nameGroup;");
		writer.println("\t public atomicGroup(CSProcess[] parArray, String group){");
		writer.println("\t\t this.parArray=parArray;");	
		writer.println("\t\t nameGroup=group;");
		writer.println("\t }");	
		writer.println("\t @Override");	
		writer.println("\t public void run() {");
		writer.println("\t\t Parallel par = new Parallel(parArray);");	
	...
	public static void createChannelValueClass() throws IOException{
		String fileName = "ChannelValue.java";
		@SuppressWarnings("resource")
		PrintWriter writer = new PrintWriter(fileName, "UTF-8");
		new File(fileName).createNewFile();	
		writer.println("import org.jcsp.lang.Any2AnyChannel;");
		writer.println("import org.jcsp.lang.Channel;");
		writer.println("import org.jcsp.lang.One2OneChannel;");
		writer.println("import org.jcsp.lang.One2OneChannelSymmetric;");
		writer.println("public class ChannelValue {");
		writer.println("\t public String type;");
		writer.println("\t public	One2OneChannel channel = Channel.createOne2One();");
		writer.println("public ChannelValue(){");	
		writer.println("}");
		writer.println("}");
		writer.close();
	}
	...
	public static void handleInExternal(Node in, PrintWriter writer) throws IOException{
	    ...
	    if(!alreadyExists){
    		...
    		if(alreadyExists){
        		writer.println("\t\t\t String name" + inPlace + " =  JOptionPane.showInputDialog(null, \"Communication with an external system, throu channel " + throughChannel + " - Give the value of " + inPlace + ": \");" );	
    			/* Allagi edw, den mporei na dexetai mesw toy xristi kapoio kanali
    			 * writer.println("\t\t\t " + inPlace + " = name" + inPlace +" ; " );   	*/		    			
	...
	public static void ReplInParSResNP(Node in, PrintWriter writer) throws IOException{
    	...
    	if(baseTypes.contains(typeInplace)){
    		if(alreadyExists){
    			...
    			writer.println("\t\t\t System.out.println(\"Input action through: " + throughChannel + " Placeholder: " + inPlace + " \");");
	...
	public static void handleOut(Node out, PrintWriter writer) throws IOException{
		...
		writer.println("\t\t\t "+thC + ".channel.out().write( " + whatC + " );");
		writer.println("\t\t\t System.out.println(\"Output action through: " + thC + " Placeholder: " + whatC + " \");");
